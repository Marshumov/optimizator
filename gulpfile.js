...
function images(){
	return gulp.src('./src/images/**/*.{png,jpg}')
				...
				//	path.basename = path.basename.replace(' ', '').replace('-', '').replace('_', '');
					...
					.pipe(debug({title: 'building images:', showFiles: true}))
...
function imagesWebp(){
			gulp.task('default', () =>
		    gulp.src('./src/images/**/*')
						.pipe(debug({title: 'building webp:', showFiles: true}))
		        .pipe(webp())
		        .pipe(gulp.dest('./build/images'))
		);
}
...
let build = gulp.series(clear,
	gulp.parallel(styles, img, js, html, font, css, video, php, json, imgAfiv, imgWebp, imagesAfiv, images , imagesWebp)
);
